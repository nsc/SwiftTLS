...
extension _BigInt {
    ...
    init<T : FixedWidthInteger>(_ parts : [T], normalized: Bool = true, negative: Bool = false)
    {
        let numberInPrimitiveType = MemoryLayout<Word>.size/MemoryLayout<T>.size
        
        switch numberInPrimitiveType {
        case 0:
            ...
            for a in parts
            {
                let shift = UInt64(8 * MemoryLayout<Word>.size)
                var mask : UInt64 = (0xffffffffffffffff >> UInt64(64 - shift))
                for i in 0 ..< n
                {
                    let part : Word = Word((UInt64(a) & mask) >> (UInt64(i) * shift))
                    number.append(part)
                    mask = mask << shift
                }
            }
    ...
    func asBigEndianData() -> [UInt8] {
        return _BigInt<UInt8>(words, normalized: false)._data.reversed()
    }
    ...
    func isBitSet(_ bitNumber : Int) -> Bool
    {
        ...
        guard wordNumber < self.words.count else {
            return false
        }
        
        return (UInt64(self.words[wordNumber]) & (UInt64(1) << UInt64(bit))) != 0
    }
    
    static func random(_ max : _BigInt) -> _BigInt
    {
        let num = max.words.count
        var words = [Word](repeating: 0, count: num)

        words.withUnsafeMutableBufferPointer { arc4random_buf($0.baseAddress, num * MemoryLayout<Word>.size); return }
